...
def load_image(path):
    ...
    img = image.load_img(img_path, target_size=(109, 89))
...
def modify_backprop(model, name):
    ...
    with g.gradient_override_map({'Relu': name}):
        ...
        new_model = load_model('../santa_not_santa.model')
    return new_model
...
def grad_cam(input_model, image, category_index, layer_name):
    ...
    cam = cv2.resize(cam, (89, 109))
    cam = np.maximum(cam, 0)
    print("cam", list(map(lambda x: float(x), cam[:, 0]))) # ???? como que aqui explota
    heatmap = cam / np.max(cam)

    print(weights)
    # imprime los pesos e imprime cam despues del maximum (linea 126), por que pasa esto?
